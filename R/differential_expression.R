...
#' @param logfc.threshold Only return results with a DE exceeding this
...
MAST_DE_rand_effect_seurat = function(
  ...
  if (!is.null(n_cores)) old_opt <- options(mc.cores=4)
  ...
  #======== get lognormalised data  =============================

  mat_data = as.matrix(Seurat::GetAssayData(object = object, assay=assay, slot=slot))

  #======== subset features ===============================

  if (!is.null(features)) mat_data <- mat_data[rownames(mat_data) %in% features, ]
  ...
  #======== check min.pct and min.cells.feature ============

  # check in each subset
  if (!is.null(min.pct)) {
    logical.min.pct.cells.1 = apply(X=mat_data[,logical.cells.1], MARGIN=1, FUN=function(generow) {sum(generow>0)/length(generow) > min.pct})
    logical.min.pct.cells.2 = apply(X=mat_data[,logical.cells.2], MARGIN=1, FUN=function(generow) {sum(generow>0)/length(generow) > min.pct})
    mat_data = mat_data[logical.min.pct.cells.1 | logical.min.pct.cells.2,]
    #features = rownames(mat_data)
  }
  ...
  cells.1 = colnames(mat_data)[idx.cells.1]
  cells.2 = colnames(mat_data)[idx.cells.2]
  mat_data = mat_data[,c(idx.cells.1,idx.cells.2)]

  #======== filter out features that are zero in all remaining cells =======

  mat_data = mat_data[apply(X = mat_data, MARGIN=1, FUN = sum)>0,]
  ...
  # random var
  df_coldata[[random_effect.var]] <- as.factor(object@meta.data[c(idx.cells.1,idx.cells.2),random_effect.var])

  # check whether random var levels overlap entirely with test condition
  for (re_lvl in levels(df_coldata[[random_effect.var]])) {
    if (all(df_coldata[[random_effect.var]]==re_lvl & 1:nrow(object@meta.data) %in% idx.cells.1)) {
      stop(paste0("random_effect.var level ", re_lvl, " overlaps entirely with ident.1"))
    }
    else if (all(df_coldata[[random_effect.var]]==re_lvl & 1:nrow(object@meta.data) %in% idx.cells.2)) {
      stop(paste0("random_effect.var level ", re_lvl, " overlaps entirely with ident.2"))
    }
  }

  #======== prep  feature data =========================
  df_feature = data.frame("primerid"=rownames(mat_data)) # primerid is hardcoded feature name in MAST

  #======== prep SingleCellAssay object ============================

  expr = quote(MAST::FromMatrix(exprsArray=mat_data,
                                 check_sanity = TRUE,
                                 cData=df_coldata,
                                 fData=df_feature))
  ...
  fmla <- as.formula(
    object = paste0(" ~ condition + cngeneson", str_plus, paste(latent.vars, collapse = "+"), " + (1|", random_effect.var, ")")
  )
  ...
  df_out = setDF(summaryCond$datatable[contrast=='conditionGroup1' & component=='logFC', .(primerid, coef, ci.hi, ci.lo)])
  df_out$p_val = summaryCond$datatable[contrast=='conditionGroup1' & component=='H', `Pr(>Chisq)`]
  rownames(df_out) = summaryCond$datatable[contrast=='conditionGroup1' & component=='H', primerid]

  # only return positive logFC and/or over set threshold
  logfc.threshold = if (is.null(logfc.threshold)) 0 else logfc.threshold
  df_out = df_out[df_out$coef > logfc.threshold,]

  # reset mc.cores option
  options(mc.cores=old_opt$mc.cores)
